...
# Evaluates the individuals in the population based on distance and time
class MarioProblem(ElementwiseProblem):
    
    def __init__(self):
        super().__init__(n_var=data.size, n_obj=2, xl=0, xu=3, type=anp.integer)

    def _evaluate(self, x, out, *args, **kwargs):

        dist, time, death = MarioExample().playGame(x, False)

        out["F"] = anp.column_stack([dist, time])
...
class MarioDisplay(Display):
    ...
    def _do(self, problem, evaluator, algorithm):
        ...
        self.output.append("Dist", np.mean(algorithm.pop.get("F")[:, 0]))
        self.output.append("Time", np.mean(algorithm.pop.get("F")[:, 1]))